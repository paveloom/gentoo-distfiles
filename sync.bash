...
download_deps_js()
{
    ...
    if [[ ! -f package.json ]]; then
        fatal "there is no \`package.json\` at the specified path"
    fi

    info "downloading the dependencies (\`${r["method"]} install\`)..."

    case "${r["method"]}" in
    "yarn")
        local node_modules_dir="$temp_dir/$deps_dir_name/node_modules"

        if ! ret=$(
            yarn install \
                --frozen-lockfile \
                --modules-folder "$node_modules_dir" 2>&1
        ); then
            error "$ret"
            fatal "failed to download the dependencies"
        fi
...
download_deps()
{
    ...
    "js") download_deps_js record revision "$temp_dir" "$deps_dir_name" ;;
